...
def _recharge_account(page: Page, logger: logging.Logger, count: int, account_id: str, order_id, task_id, wallet_id):
    ...
    try:
        ...
        if "successful" in result:
            ...
            restore_wallet_balance(wallet_id, count)
            ...
            insert_log("info", "Backend balance insufficient - Wallet balance restored", source_url=str(page.url), backend_id=BACKEND_ID, account_id=_.id, task_id=task_id)
            send_email(
            ...
            update_order_automation_status(order_id, "failed")
            insert_log("warning", f"Unknown error for recharge: {account_id} - Wallet balance restored", source_url=str(page.url), backend_id=BACKEND_ID, account_id=_.id, task_id=task_id)
        else:
            restore_wallet_balance(wallet_id, count)
            logger.info("Wallet balance restored")
            ...
            insert_log("warning", f"Unexpected recharge response: {result} - Wallet balance restored", source_url=str(page.url), backend_id=BACKEND_ID, account_id=_.id, task_id=task_id)
    except PlaywrightTimeoutError:
        restore_wallet_balance(wallet_id, count)
        logger.info("Wallet balance restored")
        ...
        update_order_status(order_id, "failed")
        update_wallet_detail_status(order_id, "failed")
        insert_log("warning", f"Failed to detect result after recharge on {BACKEND_NAME} - Wallet balance restored", source_url=str(page.url), backend_id=BACKEND_ID,
...
def _freeplay_account(page: Page, logger: logging.Logger, count: int, account_id: str, task_id, t, id_to_update):
    ...
    try:
        ...
        if "successful" in result:
            ...
            insert_log("info", "Backend balance insufficient", source_url=str(page.url), backend_id=BACKEND_ID, account_id=_.id, task_id=task_id)
            update_automation_result(task_id=task_id, status="failed", description=f"Backend balance insufficient on {BACKEND_NAME}")
            return
        elif "unknown" in result:
            logger.warning("Unknown error.")
...
@with_persistent_browser
def action_recharge_account(page: Page, count: int, account_id: str, order_id, task_id, backend, wallet_id):
    backend = get_backend(BACKEND_NAME)
    _ = get_backend_account(account_id)
    ensure_directories(DATA_DIR, CAPTCHA_DIR, LOGS_DIR)
    logger = get_backend_logger(BACKEND_NAME, LOGS_DIR)
    ...
    try:
        ...
        _login_and_navigate(page, logger, backend, task_id)
        _recharge_account(page, logger, count, account_id, order_id, task_id, wallet_id)
    except (PlaywrightTimeoutError, Exception) as e:
        restore_wallet_balance(wallet_id, count)
...
def action_read_account(page: Page, account_id: str, task_id, backend):
    ...
    try:
        insert_log(
            "info",
            f"Initiating read for account ID {account_id} on backend '{BACKEND_NAME}'", source_url=str(page.url), backend_id=BACKEND_ID, task_id=task_id
        )
        _login_and_navigate(page, logger, backend, task_id)
