...
class JuwaAPIError(Exception):
    def __init__(self, code: int, msg: str):
        super().__init__(f"juwa api error (code={code}): {msg}")
        self.code = code
        self.msg = msg
...
class JuwaClient:
    ...
    def __init__(
        ...
        if not agent_id or not secret_key or not base_url:
            raise ValueError(
                "JuwaClient requires agent_id, secret_key, and base_url "
                "(seed backend_games.api_* columns for juwa)."
            )
        self.agent_id = str(agent_id)
        self.secret_key = str(secret_key)
        self.base_url = base_url.rstrip("/")
        self.timeout = timeout
        self.logger = logger or logging.getLogger("casino_automation.juwa.api")
    ...
    def _request(
        ...
        self.logger.debug("juwa api -> %s", endpoint)
        ...
        if not isinstance(data, (dict, list)):
            data = {}
        self.logger.debug("juwa api <- %s code=%s msg=%s", endpoint, code, msg)
...
def build_client_from_backend(backend, logger: Optional[logging.Logger] = None) -> JuwaClient:
    return JuwaClient(
