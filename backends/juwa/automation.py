...
from common.utils.db_actions import get_backend, insert_backend_account, insert_log, update_game_id_by_username, \
    update_order_automation_status, update_automation_result, mark_freeplay_transferred
...
def action_create_account(page: Page, task_id, backend):
    ...
    try:
        ...
        update_automation_result(task_id=task_id, status="failed", description=f"Account creation failed. {e}")
    finally:
...
def action_recharge_account(page: Page, count: int, account_id: str, order_id, task_id, backend):
    ...
    try:
        ...
        update_automation_result(task_id=task_id, status="failed", description=f"Error during account recharge. {e}")
    finally:
...
@with_persistent_browser
def action_freeplay_account(page: Page, count: int, account_id: str, order_id, task_id, backend):
    backend = get_backend(BACKEND_NAME)
    ensure_directories(DATA_DIR, CAPTCHA_DIR, LOGS_DIR)
    logger = get_backend_logger(BACKEND_NAME, LOGS_DIR)
    logger.info("Recharge-account action started: account_id=%s, count=%d", account_id, count)
