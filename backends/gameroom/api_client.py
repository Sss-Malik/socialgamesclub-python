"""
Gameroom agent API client.

Replaces the previous browser-driven automation. The full endpoint spec
this implementation targets lives at:

    /Applications/development/gameroom-standalone/gameroom_api_findings.md

Authentication model (per spec):
    - POST /api/login → JWT bearer (~6h lifetime).
...
Session persistence:
    The JWT is cached in the existing `backend_sessions` table so it
    survives across workers and process restarts. Redis-backed
    `acquire_login_lock` prevents a thundering herd when several workers
    discover an expired token simultaneously.
"""
...
class GameroomAPIError(Exception):
    def __init__(self, status_code: int, message: str, code: Optional[int] = None):
        super().__init__(f"gameroom api error (status_code={status_code}): {message}")
...
class GameroomClient:
    ...
    def __init__(
        ...
        *,
        ...
        if not base_url or not username or not password:
            raise ValueError(
                "GameroomClient requires base_url, username, and password."
            )
        ...
        self.logger = logger or logging.getLogger("casino_automation.gameroom.api")
        ...
        # The spec doesn't pin the response shape for /api/agent/getMoney, so
        # callers can fall back to this value if parsing getMoney fails.
    ...
    def ensure_token(self) -> None:
        ...
        if db_session and db_session.token and db_session.expires:
            ...
            if exp and time.time() < exp - self._EXPIRY_GUARD_SECS:
                ...
                self.logger.debug(
                    "gameroom: reusing cached JWT session id=%s (exp=%s)",
                    db_session.id, exp,
                )
                return
    ...
    def _login(self) -> None:
        ...
        if not acquire_login_lock(self.backend_name):
            self.logger.info(
                "gameroom: login lock held by another worker; waiting for session."
            )
            ...
            if not session or not session.token:
                raise GameroomAPIError(0, "Timed out waiting for login from another worker")
        ...
        try:
            ...
            if sc != _OK:
                raise GameroomAPIError(sc, str(body.get("message", "login failed")))

            token = body.get("token")
            expires_time = body.get("expires_time")
            if not token or not expires_time:
                raise GameroomAPIError(sc, "login response missing token/expires_time")
            ...
            self._db_session_id = db_session.id
            self.logger.info(
                "gameroom: login successful, persisted session id=%s (exp=%s)",
    ...
    def _request(
        ...
        *,
        ...
        self.logger.debug("gameroom api -> %s %s", method, path)
        ...
        self.logger.debug(
            "gameroom api <- %s status_code=%s code=%s msg=%s",
            path, sc, code, msg,
        )

        if sc == _TOKEN_DEAD and _retry_on_410:
            self.logger.warning(
                "gameroom: status_code=410 on %s; re-logging in and retrying once.",
                path,
            )
            invalidate_latest_session(self.backend_name)
            self._token = None
    ...
    def agent_balance(self) -> Tuple[int, str, Dict[str, Any]]:
        """POST /api/agent/getMoney → returns body containing 'money'."""
    ...
    def reset_password(
        ...
        *,
        ...
        """POST /api/player/reset. Note: reset password rule differs from create
        (must contain upper + lower + symbol — see spec §4.8)."""
        data = {
            "id": str(player_id),
            "password": password,
            "password_confirmation": password,
...
def build_client_from_backend(backend, logger: Optional[logging.Logger] = None) -> GameroomClient:
    """Construct a GameroomClient from a BackendGame row.

    Prefers DB-seeded values; falls back to config constants if absent.
    """
    from backends.gameroom.config import LOGIN_URL, USERNAME, PASSWORD, BACKEND_NAME

    base_url = backend.api_base_url or _derive_api_base(backend.backend_url or LOGIN_URL)
...
def _derive_api_base(admin_url: str) -> str:
    """Strip any trailing /admin path to recover the bare API host."""
