...
from common.utils.db_actions import get_backend, insert_backend_account, insert_log, update_game_id_by_username, \
    ...
    process_recharge_operation, update_freeplay, process_freeplay_operation, insert_log_and_update_automation_result
    ...
    try:
        ...
        if "successful" in result:
            ...
            process_recharge_operation(
                ...
                log_data={
                    "type": "warning",
                    "description": f"Unexpected recharge response: {result} - Wallet balance restored"
                },
                order_status="failed",
                automation_status="failed",
                automation_result_fields={
...
def _freeplay_account(page: Page, logger: logging.Logger, count: int, account_id: str, task_id, t, id_to_update, freeplay_id):
    ...
    try:
        ...
        if "successful" in result:
            ...
            insert_log_and_update_automation_result(
                log_type="warning",
                log_description=f"Unknown error for recharge: {account_id} ",
                task_id=task_id,
                source_url=str(page.url),
                backend_id=BACKEND_ID,
                account_id=_.id,
            ...
            insert_log_and_update_automation_result(
                log_type="warning",
                log_description=f"Unexpected recharge response: {result}",
                task_id=task_id,
                source_url=str(page.url),
                backend_id=BACKEND_ID,
                account_id=_.id,
...
def action_recharge_account(page: Page, count: int, account_id: str, order_id, task_id, backend, wallet_id, amount_to_deduct):
    ...
    try:
        ...
        insert_log("info", "Recharge account action completed", source_url=str(page.url), backend_id=BACKEND_ID, account_id=backend_account.id, task_id=task_id)
