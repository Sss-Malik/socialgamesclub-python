...
def _create_single_account(page, logger: logging.Logger):
    ...
    try:
        ...
        try:
            page.wait_for_selector("#mb_con", timeout=10_000, state="attached")
            message_text = page.query_selector("#mb_msg").inner_text().lower() if page.query_selector("#mb_msg") else ""
            if any(phrase in message_text for phrase in ACCOUNT_SUCCESS_MSG):
                logger.info("✅ Account created successfully.")
                save_credentials(account_id, password, logger, DATA_DIR)
                ok_button = page.wait_for_selector("#mb_btn_ok", timeout=10_000, state="attached")
                if ok_button:
                    ok_button.click()
            else:
...
def _recharge_account(page, logger: logging.Logger, count: int, account_id):
    try:
        ...
        recharge_frame.click("a:has-text('Recharge')")
