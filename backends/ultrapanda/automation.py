    ...
    for attempt in range(5):
        ...
        try:
            ...
            if "cannot exceed current points" in text:
                ...
                update_automation_result(task_id=task_id, status="failed", description="Unexpected response.")
                insert_log("warning", f"Unexpected withdrawal response: {text}", source_url=str(page.url))
        except PlaywrightTimeoutError:
            logger.exception("No dialog appeared after setting score.")
            update_automation_result(task_id=task_id, status="failed", description="Failed to detect dialog after withdrawal.")
