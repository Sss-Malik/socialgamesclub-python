"""
Cash Machine 777 backend: direct HTTP API implementation.

Ported from backends/gameroom/automation.py — Cash Machine 777 is the same
white-label "layui" agent panel and shares the JWT + form-urlencoded JSON API.
Preserves all DB side effects, webhook trigger points, email sends, and
response / description strings so callers (Laravel etc.) need no changes.

See ./cash_machine_777_api_findings.md for the full endpoint spec and the live
verification log.
"""
...
from backends.cashmachine.utils.credentials import generate_credentials
from backends.cashmachine.api_client import (
    CashMachineAPIError,
...
def _resolve_player_id(backend_account, client, logger: logging.Logger) -> str:
    """Return the cashmachine player id for this BackendAccount.
    ...
    if sc != _OK:
        raise CashMachineAPIError(sc, f"userList failed: {msg}")
...
def _fetch_balances(player_id: str, client) -> tuple:
    ...
    if sc != _OK:
        raise CashMachineAPIError(sc, msg)
...
def _extract_agent_money(body, fallback=None):
    ...
    Verified CM777 shape returns the balance directly as `data` (a string);
    the int/float/str branch below handles that. The other shapes and the
    login-money fallback are kept for robustness.
    """
...
def action_read_backend(task_id, backend, **_):
    ...
    try:
        ...
        if balance_value is None:
            raise CashMachineAPIError(sc, "Could not extract agent money from response")
...
def action_reset_password(account_id: str, task_id, backend, **_):
    ...
    try:
        ...
        # Reset password rule differs from create — must contain upper + lower
        # + a special symbol. generate_credentials(use_special_char=True)
