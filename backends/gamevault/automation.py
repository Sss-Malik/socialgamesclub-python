"""
Gamevault backend: direct HTTP API implementation.

Replaces the previous Playwright automation. Preserves all DB side effects,
webhook trigger points, email sends, and response / description strings so
Laravel needs no changes.

See /Users/saud/.claude/plans/query-context-the-primary-functional-whistle.md
for the transition plan this file implements.
"""
...
from backends.gamevault.api_client import (
    GamevaultAPIError,
...
# ---------------------------------------------------------------------------
# Error-code classification (see plan's "Error-code mapping" table)
# ---------------------------------------------------------------------------
...
def _resolve_user_id(backend_account, client, logger: logging.Logger) -> str:
    """Return gamevault's numeric user_id as a string.

    Prefers the cached BackendAccount.game_id; falls back to getUserID and
    backfills (first-writer-wins) when absent.
    """
    ...
    if code != 0 or not data.get("user_id"):
        raise GamevaultAPIError(code, f"Could not resolve user_id for {backend_account.username}: {msg}")
...
def _call_with_stale_id_retry(api_call, backend_account, client, logger: logging.Logger):
    """Invoke an API call; if it returns code 8 (invalid user id), force
    re-resolve via getUserID once and retry. Returns (code, msg, data).
    """
    code, msg, data = api_call()
    if code != 8:
        return code, msg, data

    logger.warning("gamevault returned code 8 for %s; re-resolving user_id", backend_account.username)
        ...
        for i in range(count):
            ...
            for attempt in range(CREATE_ACCOUNT_MAX_RETRIES):
                ...
                if code == 18:
                    insert_log(
                        "error",
                        f"gamevault rejected account format: {account_id} ({msg})",
                        source_url=None, backend_id=BACKEND_ID, task_id=task_id,
                    )
                    hard_stop = True
                    break
...
def action_read_backend(task_id, backend, **_):
    ...
    try:
        ...
        if code != 0:
            raise GamevaultAPIError(code, msg)
...
def action_read_account(account_id: str, task_id, backend, **_):
    ...
    try:
        ...
        # Preserve today's JSON key set; fields the API doesn't expose get "".
        out = {
            "id": user_id,
            "account": account_id,
        ...
        # Persist game_id mapping for any pre-cutover rows that lacked it.
...
def action_recharge_account(
    ...
    try:
        ...
        if code == 6:
            ...
            result_description = f"Backend balance insufficient for {BACKEND_NAME} - Wallet balance restored"
        elif code == 9:
            ...
            result_description = f"Form submission error on {BACKEND_NAME}. Try again later - Wallet balance restored"
        else:
...
def action_freeplay_account(
    ...
    try:
        ...
        if code == 6:
            ...
            log_description = f"Form submission error. Please try again"
...
# Dispatcher uses "create-account-user" → action_create_account_user; parity with today.
